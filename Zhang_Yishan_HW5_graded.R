...
function3<-function(x){
  #give columns names 
  colnames(x)
  #Stuart- Technically each should be written on its own line. One way to do this is to use
  #writeLines(colnames(x))
}
...
#Stuart- you should include a require() or library() statement for plyr
#You also don't just apply this to factor columns, the table() function has a cleaner presentation.
...
function7<-function(x){
  ...
  x[x==0]<-NA #Stuart- Why are you creating NAs?
...
#Stuart- You are correctly naming the rows, but you are not correctly caclulating the correlation. There should
#be a single correlation value for each pair of variables.
